...
class Order(db.Model):
    ...
    # VERSION 2 END
    # VERSION 4 START
    receipt_emailed_at = db.Column(db.DateTime, nullable=True)
    # VERSION 4 END
...
class AuditLog(db.Model):
    __tablename__ = "audit_logs" # Database table name
    # Primary key for the audit log entry
    id = db.Column(db.Integer, primary_key=True)
    # ID of the user who performed the action
    actor_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=True)
    # Role of the user at the time of the action (user / organiser / admin)
    actor_role = db.Column(db.String(20), nullable=True)
    # Action performed (e.g. ORDER_PAID, RECEIPT_EMAILED)
